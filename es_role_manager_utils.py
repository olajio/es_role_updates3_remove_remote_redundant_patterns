...
class ElasticsearchRoleManager:
    ...
    # =========================================================================
    # PATTERN SUBSET AND CLEANUP METHODS
    # =========================================================================

    @staticmethod
    def is_pattern_subset(candidate: str, potential_superset: str) -> bool:
        """
        Check if candidate pattern is a subset of potential_superset pattern.
        
        A pattern A is a subset of pattern B if every index matching A also matches B.
        
        For suffix wildcard patterns (prefix*):
        - 'apm-*' is subset of 'apm*' (apm- starts with apm)
        - 'partial-restored-*' is subset of 'partial-*'
        - 'restored-filebeat-*' is subset of 'restored-*'
        
        Args:
            candidate: Pattern to check if it's a subset
            potential_superset: Pattern to check if it's a superset
            
        Returns:
            True if candidate is a subset of potential_superset
        """
        # Skip if same pattern
        if candidate == potential_superset:
            return False
        
        # Both must end with * for this simple check to work
        if not candidate.endswith('*') or not potential_superset.endswith('*'):
            return False
        
        # Extract prefixes (everything before the *)
        candidate_prefix = candidate[:-1]
        superset_prefix = potential_superset[:-1]
        
        # candidate is subset if its prefix starts with superset's prefix
        # e.g., "apm-" starts with "apm", so apm-* is subset of apm*
        return candidate_prefix.startswith(superset_prefix)

    @staticmethod
    def is_pattern_covered(pattern: str, local_patterns: Set[str]) -> bool:
        """
        Check if a pattern is covered by any local pattern.
        
        A pattern is covered if:
        - It equals a local pattern, OR
        - It is a subset of a local pattern
        
        Args:
            pattern: Pattern to check
            local_patterns: Set of local patterns to check against
            
        Returns:
            True if pattern is covered by any local pattern
        """
        # Direct match
        if pattern in local_patterns:
            return True
        
        # Check if pattern is a subset of any local pattern
        for local in local_patterns:
            if ElasticsearchRoleManager.is_pattern_subset(pattern, local):
                return True
        
        return False

    @staticmethod
    def find_subset_patterns(patterns: Set[str]) -> Set[str]:
        """
        Find patterns that are subsets of other patterns in the set.
        
        Args:
            patterns: Set of patterns to analyze
            
        Returns:
            Set of patterns that should be removed (are subsets of others)
        """
        to_remove = set()
        patterns_list = list(patterns)
        
        for candidate in patterns_list:
            for potential_superset in patterns_list:
                if ElasticsearchRoleManager.is_pattern_subset(candidate, potential_superset):
                    to_remove.add(candidate)
                    break  # No need to check further, already marked for removal
        
        return to_remove

    def cleanup_subset_patterns(self, role_definition: Dict) -> Tuple[Dict, Set[str]]:
        """
        Remove index patterns that are subsets of other patterns in the role.
        
        For example:
        - If role has both 'apm*' and 'apm-*', removes 'apm-*'
        - If role has both 'partial-*' and 'partial-restored-*', removes 'partial-restored-*'
        
        Only considers:
        - Local patterns (no ':' - remote patterns are handled separately)
        - Simple patterns (no ',' - comma-separated are too complex)
        
        Args:
            role_definition: Role definition dictionary
            
        Returns:
            Tuple of (updated role definition, set of removed patterns)
        """
        updated_role = json.loads(json.dumps(role_definition))
        
        if not updated_role.get('indices'):
            return updated_role, set()
        
        # Collect all local simple patterns
        all_local_patterns = set()
        for entry in updated_role.get('indices', []):
            for name in entry.get('names', []):
                # Only consider local (no ':') and simple (no ',') patterns
                if ':' not in name and ',' not in name:
                    all_local_patterns.add(name.strip())
        
        # Find subset patterns
        subsets_to_remove = self.find_subset_patterns(all_local_patterns)
        
        if not subsets_to_remove:
            return updated_role, set()
        
        self.logger.debug(f"Removing subset patterns: {subsets_to_remove}")
        
        # Remove subsets from each indices entry
        for entry in updated_role['indices']:
            original_names = entry.get('names', [])
            entry['names'] = [n for n in original_names if n.strip() not in subsets_to_remove]
        
        # Remove empty entries
        updated_role['indices'] = [
            e for e in updated_role['indices'] 
            if e.get('names')
        ]
        
        return updated_role, subsets_to_remove

    def remove_remote_index_patterns(
        self, 
        role_definition: Dict, 
        local_patterns: Set[str]
    ) -> Tuple[Dict, Set[str]]:
        """
        Remove remote index patterns from a CCS role where the base pattern
        is covered by a local pattern.
        
        A remote pattern like 'prod:filebeat-*' is removed if:
        - 'filebeat-*' exists in local_patterns, OR
        - 'filebeat-*' is a subset of a pattern in local_patterns (e.g., 'filebeat*')
        
        For comma-separated patterns like 'prod:traces-apm*,prod:logs-apm*':
        - Only removes if ALL base patterns are covered locally
        
        Args:
            role_definition: Role definition dictionary
            local_patterns: Set of local patterns that exist in the role
            
        Returns:
            Tuple of (updated role definition, set of removed remote patterns)
        """
        updated_role = json.loads(json.dumps(role_definition))
        removed_patterns = set()
        
        if not updated_role.get('indices'):
            return updated_role, removed_patterns
        ...
        for entry in updated_role['indices']:
            ...
            for name in entry.get('names', []):
                ...
                if ':' not in name:
                    # Local pattern, keep it
                    names_to_keep.append(name)
                    continue
                
                # This is a remote pattern
                should_remove, base_patterns = self._should_remove_remote_pattern(name, local_patterns)
                
                if should_remove:
                    removed_patterns.add(name)
                    self.logger.debug(
                        f"Removing remote pattern '{name}' - base pattern(s) {base_patterns} covered locally"
                    )
                else:
                    names_to_keep.append(name)
            
            entry['names'] = names_to_keep
    ...
    def _should_remove_remote_pattern(
        self, 
        pattern: str, 
        local_patterns: Set[str]
    ) -> Tuple[bool, List[str]]:
        """
        Determine if a remote pattern should be removed.
        
        Args:
            pattern: Remote pattern (contains ':')
            local_patterns: Set of local patterns
            
        Returns:
            Tuple of (should_remove, list of base patterns)
        """
        if ':' not in pattern:
            return False, []
        
        if ',' in pattern:
            # Comma-separated remote patterns like 'prod:traces-apm*,prod:logs-apm*'
            parts = [p.strip() for p in pattern.split(',')]
            base_patterns = []
            ...
            for part in parts:
                if ':' in part:
                    ...
                    # Mixed format (some parts don't have cluster prefix)
                    # Be conservative and don't remove
                    return False, []
            
            # Only remove if ALL base patterns are covered
            all_covered = all(
                self.is_pattern_covered(base, local_patterns) 
                for base in base_patterns
            )
            return all_covered, base_patterns
        else:
            # Simple remote pattern like 'prod:filebeat-*'
            base = pattern.split(':', 1)[1]
            is_covered = self.is_pattern_covered(base, local_patterns)
            return is_covered, [base] if is_covered else []
