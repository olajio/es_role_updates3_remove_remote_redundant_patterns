...
def update_single_role(
    ...
    """
    Update a single role with new patterns and cleanup subset patterns
    ...
    try:
        ...
        updated_role, removed_subsets = manager.cleanup_subset_patterns(updated_role)
        if removed_subsets:
            logger.info(f"  [{cluster_name}] Removed subset patterns: {', '.join(sorted(removed_subsets))}")
...
def update_ccs_role_with_kibana(
    ...
    """
    Update a CCS role with patterns, cleanup, and Kibana privileges
    
    Order of operations:
    1. Add patterns (inject + sync)
    2. Cleanup subset patterns
    3. Remove remote index patterns
    4. Update Kibana privileges
    ...
    if dry_run:
        ...
        changes.append("subset cleanup")
        changes.append("remote pattern removal")
        logger.info(f"  [{cluster_name}] [DRY RUN] Would apply: {', '.join(changes)}")
        return True
    
    try:
        updated_role = role_def
        
        # Step 1: Add patterns if needed
        if patterns_to_add:
        ...
        # Step 3: Remove remote index patterns
        # First, get the current set of local patterns after cleanup
        local_patterns = manager.get_existing_local_patterns(updated_role)
        updated_role, removed_remotes = manager.remove_remote_index_patterns(updated_role, local_patterns)
        if removed_remotes:
            logger.info(f"  [{cluster_name}] Removed remote patterns: {', '.join(sorted(removed_remotes))}")
        
        # Step 4: Add Kibana privileges if needed
